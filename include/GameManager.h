...
public:
	...
	void dracula_ability(Player& player1);
	Complet_Needs take_needs(Player& player1, Player& player2, Card& card, Hero* heroteam = nullptr, Hero* herotarget = nullptr);
	void movement(Player* player1, Hero* hero, int moveMax = 0, int moveMin = 0);
	void initial_position(Player& player1, Player& player2);
	ShowActionMenu complet_action_menu(Player& player1, Player& player2);
